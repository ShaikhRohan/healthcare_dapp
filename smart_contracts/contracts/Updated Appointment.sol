...
contract Appointments {
    ...
    event bookAppointment(address sender,string patient, string doctor, uint doctorNo, uint slotNo, string date, uint256 timestamp, string pdfuri);
    ...
    function addToBlockchain(string memory patient,string memory doctor, uint doctorNo, uint slotNo, string memory date, string memory pdfuri) public {
        ...
        appointments.push(Appointment(msg.sender,patient, doctor, slotNo, doctorNo, date, block.timestamp ,pdfuri));

        emit bookAppointment(msg.sender,patient, doctor, doctorNo, slotNo, date, block.timestamp, pdfuri);
    } 
